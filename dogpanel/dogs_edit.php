...
$errors = [];
$name = trim($_POST['name'] ?? $dog['name']);
$breed_id = trim($_POST['breed_id'] ?? $dog['breed_id']);
$age = trim($_POST['age'] ?? $dog['age']);
$price = trim($_POST['price'] ?? $dog['price']);
$description = trim($_POST['description'] ?? $dog['description']);
$current_photo = $dog['main_photo'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!isset($allowed[$type])) {
            ...
            if (!move_uploaded_file($_FILES['photo']['tmp_name'], $dest)) {
                ...
                if ($current_photo && is_file(__DIR__ . '/uploads/dogs/' . $current_photo)) {
                    @unlink(__DIR__ . '/uploads/dogs/' . $current_photo);
                }
    ...
    if (!$errors) {
        $stmt = $pdo->prepare("UPDATE dogs SET name = ?, breed_id = ?, age = ?, price = ?, description = ?, main_photo = ? WHERE id = ?");
        $stmt->execute([
            $name,
...
?>
<h1 style="margin-bottom:16px;">Редактировать собаку</h1>
...
<form method="post" enctype="multipart/form-data" class="form-card">
    ...
    <div class="form-row">
        <label for="photo">Новое фото (если нужно заменить)</label>
        <input type="file" id="photo" name="photo" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp">
        <?php if ($current_photo): ?>
            <div style="margin-top:8px;">
                <span style="font-size:12px;color:#9f9fd0;">Текущее фото:</span><br>
                <img src="uploads/dogs/<?php echo h($current_photo); ?>" alt="" style="width:120px;height:120px;object-fit:cover;border-radius:12px;margin-top:4px;">
            </div>
        <?php endif; ?>
    </div>
